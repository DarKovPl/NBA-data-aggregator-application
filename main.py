...
def players_stats(name):
    ...
    player_stats.streamline_players_stats_data_frame()
    player_stats.convert_imperial_to_metric()
    player_stats.filter_players(name)
    print(player_stats.__str__())
...
def teams_stats(year_season, condition):
    ...
    season_teams_stats.get_all_season_teams_stats(year_season)
...
if __name__ == "__main__":
    ...
    teams_stats_parser = subparsers.add_parser(
        "teams-stats", help="This argument shows teams statistics."
    )
    teams_stats_parser.add_argument(
        ...
        choices=range(
            1979, 2022
        ),  # There is no data from 2022, so the choice ends on the year 2021.
