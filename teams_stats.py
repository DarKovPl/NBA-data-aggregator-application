...
class TeamStats(
    ...
    def get_all_season_teams_stats(self, year_season):
        for i in self.get_teams_stats(year_season):
            self.season += AllGamesInSeasonDataSchema(many=True).dump(i)

    def create_teams_names_set(self):
        teams_name_list = []
        for z in self.season:
            teams_name_list.append(
                z["visitor_team"]["full_name"]
                + f" ({z['visitor_team']['abbreviation']})"
            )
            teams_name_list.append(
                z["home_team"]["full_name"] + f" ({z['home_team']['abbreviation']})"
            )
    ...
    def count_statistics_for_teams(self):
        ...
        for match, team in itertools.product(self.season, self.result_list):
        ...
        self.dict_list_with_no_key = [
            v for k in self.result_list for v in list(k.values())
        ]
    ...
    def write_teams_stats_to_csv(self):
        for k in self.result_list:
            ...
            if self.check_teams_stats_csv_file():
                df.to_csv(
                    self.teams_stats_csv_file, header=False, mode="a", index=False
                )
            else:
